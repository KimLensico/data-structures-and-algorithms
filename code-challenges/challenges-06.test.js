...
const templatingWithMustache = () => {
  // Write a function named templatingWithMustache that uses mustache --done by challenge-- 
  // create the markup templates for each of the characters --done by challenge-- 
  ...
  characters.forEach(character => {
    // grab the template
    // create a variable that creates a template 
    let $template = $('#template').html();
    // use mustache to apply a character to the template
    newArray.push(Mustache.render($template, character));
    // push result into the new array
  });
  return newArray;

  // let $getArray = $('#template').html();
  // let $tag = $(`<script>${getArray}</script>`);
  // $tag.find('h2').text(characters.name);
  // $tag.find('h3').text(characters.spouse);
  // $tag.find('#children').text(characters.children);
  // $tag.find('p').text(characters.house);
};
...
const getHouses = (arr) => {
  let houses = [];
  // grab the names of all the houses in the data set
  arr.forEach(character => {
    // push the names of all the houses in the data set
...
const hasChildrenValues = (arr, character) => {
  // has children values will return true
  // null children will return false
  let result = false;
  arr.forEach(person => {
    // we're getting the values of the person

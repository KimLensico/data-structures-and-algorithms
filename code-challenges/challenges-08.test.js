...
const filterStringsWithVowels = (arr) => {
  // filters through an array with regex expression
  return arr.filter(name => /[aeiou]/gmi.test(name));
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(elements => {
    // filtering elements through an array so that forbiddenValues are not included 
    if (!forbiddenValues.includes(elements)) {
      // next line returns the value to array
      return elements;
    }
  });
};

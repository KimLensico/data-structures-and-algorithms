...
const templateWithJQuery = () => {
  // create a function 
  // get the html id 'template' from the DOM
  // copy the contents
  // append the contents into the DOM
  starWarsPeople.forEach(people => {
    let getArray = $('#template').html();
    //  create a variable that will grab an element in html
    let $element = $(`<section>${getArray}</section>`);
    // find the tags that we want to append to 
    $element.find('h2').text(people.name);
    $element.find('h3').text(people.height);
    $element.find('p').text(people.eye_color);
    $(`main`).append($element);
  });
};
...
const howMuchPencil = (str) => {
  let result = [];
  // function uses slice within a loop 
  // function continues until nothing is left 
  for (let i = 0; i <= str.length; i++) {
    // we push result to the array
    // so we push into the area and then we use the push method
    // in the push method we are slicing our string incrementally
